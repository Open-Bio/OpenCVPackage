...
class OpenCvLib(FunctionLibraryBase):
    ...
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "Inputs", NodeMeta.KEYWORDS: []}
    )
    ...
    @IMPLEMENT_NODE(
        ...
        meta={
            NodeMeta.CATEGORY: "Inputs",
            NodeMeta.KEYWORDS: [],
            NodeMeta.CACHE_ENABLED: False,
        },
    )
    # Reads next Video Frame from an already loaded video with cv_ReadVideo
    def cv_ReadNextFrame(
        video=("VideoPin", ""), skip_frame=("IntPin", 0), img=(REF, ("ImagePin", None))
    ):
        """Return a frame of the loaded image."""
    ...
    def cv_ReadNextTwoFrame(
        video=("VideoPin", ""),
        img=(REF, ("ImagePin", None)),
        next_img=(REF, ("ImagePin", None)),
    ):
        """Reads next 2 Video Frames from an already loaded video with cv_ReadVideo"""
        ret, frame = video.read()
        if not ret:
            video.video_capture.set(cv2.CAP_PROP_POS_FRAMES, 0)
    ...
    # Reads a specific Video Frame from an already loaded video with cv_ReadVideo
    def cv_ReadVideoFrame(
        video=(
            "VideoPin",
            0,
        ),
    ...
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "Converters", NodeMeta.KEYWORDS: []}
    )
    # Return a random frame of x,y
    def cv_BGR2RGB(input=("ImagePin", None), img=(REF, ("ImagePin", None))):
    ...
    def cv_BGR2GRAY(input=("ImagePin", None), img=(REF, ("ImagePin", None))):
        img(cv2.cvtColor(input, cv2.COLOR_BGR2GRAY))

    ###################     DEBUG NODES      ##########################################################
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "Debug", NodeMeta.KEYWORDS: []}
    )
    def cv_Histogram(input=("ImagePin", None), img=(REF, ("ImagePin", None))):
        bgr_planes = cv2.split(input)
        histSize = 256
    ...
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "OpenCl", NodeMeta.KEYWORDS: []}
    )
    # Return a random frame of x,y
    def cv_convertToGPU(img=("ImagePin", None), gpuImg=(REF, ("ImagePin", None))):
        gpuImg(cv2.UMat(img))
    ...
    # Return a random frame of x,y
    def cv_convertToCPU(gpuImg=("ImagePin", None), img=(REF, ("ImagePin", None))):
        if gpuImg.__class__.__name__ == "UMat":
            img(cv2.UMat.get(gpuImg))
        else:
            img(gpuImg)
    ...
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "Color", NodeMeta.KEYWORDS: []}
    )
    def cv_HDR_AutoGamma(input=("ImagePin", None), img=(REF, ("ImagePin", None))):
        image = np.clip(np.power(input, 1.0 / 2.2), 0, 1)
    ...
    def cv_ConvertScaleAbs(input=("ImagePin", None), img=(REF, ("ImagePin", None))):
        img(cv2.convertScaleAbs(input))
    ...
    def color_filter(
        ...
        boundaries = [  # would be good to have a BGR colour picker..
            ...
            (
                [103, 86, 65],
                [145, 133, 128],
            ),  # RBG lower and upper limit for GREY (intensity)
        ]
    ...
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "Edge Detection", NodeMeta.KEYWORDS: []}
    )
    # Return a random frame of x,y
    def auto_canny(
        input=("ImagePin", None),
    ...
    def HED(
        ...
        class CropLayer(object):
            ...
            def forward(self, inputs):
                ...
                return [
                    inputs[0][:, :, self.startY : self.endY, self.startX : self.endX]
                ]
        ...
        protoPath = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "..", "res", "deploy.prototxt"
        )
    ...
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "Feature", NodeMeta.KEYWORDS: []}
    )
    def goodFeatureToTrack(
        input=("ImagePin", None),
    ...
    def Feature_Extraction(
        ...
        kp, des = extractor.detectAndCompute(input, None)
        if kp and len(kp):
    ...
    def FAST_Feature(
        ...
        kp, des = fast.detectAndCompute(input, None)
        if kp and len(kp):
            # corners = np.float32(corners)
            keypoints((kp,))
            descriptor(des)
    ...
    def BRISK_Feature(
        ...
        kp, des = brisk.detectAndCompute(input, None)
        if kp and len(kp):
            # corners = np.float32(corners)
            keypoints((kp,))
            descriptor(des)
    ...
    def AKAZE_Feature(
        ...
        kp, des = akaze.detectAndCompute(input, None)
        if kp and len(kp):
            # corners = np.float32(corners)
            keypoints((kp,))
            descriptor(des)
    ...
    def ORB_Feature_Extraction(
        input=("ImagePin", None),
        keypoints=("KeyPointsPin", 0),
        descriptor=(REF, ("DescriptorPin", 0)),
    ):
        """Takes an image and mask and applied logic and operation"""
        orb = cv2.ORB_create(nfeatures=2000)
        kp, des = orb.compute(input, keypoints.data[0])
        descriptor(des)
    ...
    def Draw_Match(
        input_1=("ImagePin", None),
        keypoints_1=("KeyPointsPin", 0),
        input_2=("ImagePin", None),
        keypoints_2=("KeyPointsPin", 0),
        matches=("FeatureMatchPin", 0),
        output=(REF, ("ImagePin", None)),
    ):
        """Takes an image and mask and applied logic and operation"""
    ...
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "Process", NodeMeta.KEYWORDS: []}
    )
    # Return a random frame of x,y
    def shape_detector(
        input=("ImagePin", None),
        ...
        for c in cnts:
            ...
            cv2.putText(
                image, text, (cX, cY), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2
            )
    ...
    def adapt_thres(
        input=("ImagePin", None),
        _mean=(REF, ("ImagePin", None)),
        _thresh=(REF, ("ImagePin", None)),
        _gaussian=(REF, ("ImagePin", None)),
    ):
        """filter a BGR color range from image"""
        image = input
    ...
    def waters_hed(
        input=("ImagePin", None),
        _thres=(REF, ("ImagePin", None)),
        _sure_bg=(REF, ("ImagePin", None)),
        img=(REF, ("ImagePin", None)),
    ):
        """filter a BGR color range from image"""
        image = input
    ...
    def cv_Threahold(
        input=("ImagePin", None),
        threshold=("IntPin", 127),
        img=(REF, ("ImagePin", None)),
    ):
        """Return a frame of the loaded image."""
        threshold, cv__threshold = cv2.threshold(
            input, threshold, 255, cv2.THRESH_BINARY
        )
        img(cv__threshold)
        ...
        if "rect" in rect.graph.keys():
            ...
            if only_rect:
                ...
                if (x + w) <= iw and (y + h) < ih:
                    img(input[y : (y + h), x : (x + w), :])

    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "Process", NodeMeta.KEYWORDS: []}
    )
    def image_crop_multi(
        img=("ImagePin", None),
        croped=(
            REF,
            (
    ...
    def image_unCrop_multi(
        originalImage=("ImagePin", None),
        croped=(
            "ImagePin",
            [],
            {PinSpecifiers.ENABLED_OPTIONS: PinOptions.ArraySupported},
    ...
    def cv_Draw_Rects(
        refImage=("ImagePin", None),
        img=(REF, ("ImagePin", 0)),
        rects=("GraphElementPin", 0),
    ):
        """Takes an image and mask and applied logic and operation"""
        rects = rects.graph
        originalImage = np.zeros(refImage.shape, dtype="uint8")

        for draw_type, draw_list in rects.items():
            if draw_type == "rect":
                for x, y, w, h in draw_list:
                    cv2.rectangle(
                        originalImage, (x, y), (x + w, y + h), (255, 255, 255), -1
                    )
    ...
    ###################     VIDEO ANALYSIS NODES      ##########################################################
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "VideoAnalysis", NodeMeta.KEYWORDS: []}
    )
    ...
    def BackgroundSubtract(
        input=("ImagePin", None),
        background_subtrator=("BackgroundSubtractorPin", 0),
        fgMask=(REF, ("ImagePin", None)),
    ):
        mask = background_subtrator.data.apply(input)
        fgMask(mask)
    ...
    @IMPLEMENT_NODE(
        returns=None,
        meta={NodeMeta.CATEGORY: "Detection and Recognition", NodeMeta.KEYWORDS: []},
    )
    def blob_detector(
        input=("ImagePin", None),
    ...
    def ocr(
        ...
        if "rect" in boxes.graph:
            for startX, startY, w, h in boxes.graph["rect"]:
                ...
                r = input[startY:endY, startX:endX]
    ...
    def gender_dnn(
        ...
        gender_proto_path = os.path.join(
            os.path.abspath(os.path.dirname(__file__)),
            "..",
            "res",
            "deploy_gender2.prototxt",
        )
        gender_caffe_path = os.path.join(
            os.path.abspath(os.path.dirname(__file__)),
            "..",
            "res",
        ...
        image = cv2.resize(input, (227, 227))
        image = image[:, :, :3]
        (h, w) = image.shape[:2]
    ...
    def classifcation_dnn(
        ...
        face_model_proto_path = os.path.join(
            os.path.abspath(os.path.dirname(__file__)),
            "..",
            "res",
        ...
        keywords_path = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "..", "res", "synset_words.txt"
        )
    ...
    def yolo_dnn(
        ...
        yolo_model_proto_path = os.path.join(
            os.path.abspath(os.path.dirname(__file__)),
            "..",
            "res",
            "yolov3-tiny.weights",
        ...
        yolo_model_path = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "..", "res", "yolov3-tiny.cfg"
        )
        keywords_path = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "..", "res", "yolov3.txt"
        )
        ...
        image = cv2.resize(input, (modelSize, modelSize))
        image = image[:, :, :3]
        (h, w) = image.shape[:2]
        ...
        blob = cv2.dnn.blobFromImage(
            image, scale, (w, h), mean=(0, 0, 0), swapRB=True, crop=False
        )
        net.setInput(blob)
    ...
    def eye_Masking(
        ...
        if isinstance(input, list):
            ...
            for imagen in input:
                ...
                eyes = eye_cascade.detectMultiScale(gray, scaleFactor, minNeighbores)
                tmp = np.zeros(imagen.shape, dtype="uint8")
                angle = 0
                if len(eyes) == 2:
            ...
            gray = cv2.cvtColor(input, cv2.COLOR_BGR2GRAY)
            eyes = eye_cascade.detectMultiScale(gray, scaleFactor, minNeighbores)
            tmp = np.zeros(input.shape, dtype="uint8")
            angle = 0
            if len(eyes) == 2:
                dirx = eyes[0][0] - eyes[1][0]
                diry = eyes[0][1] - eyes[1][1]
