...
from ..CV_classes.blend_modes import *
from ..CV_classes.imageUtils import *
...
class ImageBlendingLib(FunctionLibraryBase):
    ...
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "Blending", NodeMeta.KEYWORDS: []}
    )
    def cv_AlphaMixImages(
        FG=("ImagePin", None),
        BG=("ImagePin", None),
        Mask=("ImagePin", None),
        ...
        outImage = cv2.normalize(
            cv2.add(foreground, background), None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U
        )
        img(outImage)
    ...
    def cv_BlendImages(
        base_image=("ImagePin", None),
        overlay_image=("ImagePin", None),
        Mask=("ImagePin", None),
        blendmode=(
            "StringPin",
            0,
        ...
        modes = {
            key: value
            for key, value in BlendModes.__dict__.items()
        ...
        img(
            cv2.normalize(
                np.clip(result, 0, 1), None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U
            )
        )
