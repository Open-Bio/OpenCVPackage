...
class OpticalFlowLib(FunctionLibraryBase):
    ...
    @staticmethod
    @IMPLEMENT_NODE(
        returns=None, meta={NodeMeta.CATEGORY: "OpticalFlow", NodeMeta.KEYWORDS: []}
    )
    def OpticalFlowSparseToDense(
        previmg=("ImagePin", None),
        img=("ImagePin", None),
        grid_step=("IntPin", 8),
        k=("IntPin", 128),
        sigma=("FloatPin", 0.05),
        use_post_proc=("BoolPin", True),
        fgs_lambda=("FloatPin", 500),
        fgs_sigma=("FloatPin", 1.5),
        flowVis=(REF, ("ImagePin", None)),
        flowVisCombined=(REF, ("ImagePin", None)),
    ):
        prevFrame = previmg
        frame = img
    ...
    def OpticalFlowSF(
        ...
        img=("ImagePin", None),
        layers=("IntPin", 3),
        averaging_block_size=("IntPin", 2),
        max_flow=("IntPin", 4),
        sigma_dist=("FloatPin", 4.1),
        sigma_color=("FloatPin", 25.5),
        postprocess_window=("IntPin", 18),
        sigma_dist_fix=("FloatPin", 55.0),
        sigma_color_fix=("FloatPin", 25.5),
        occ_thr=("FloatPin", 0.35),
        upscale_averaging_radius=("IntPin", 18),
        upscale_sigma_dist=("FloatPin", 55.0),
        upscale_sigma_color=("FloatPin", 25.5),
    ...
    def LK_optical_flow(
        ...
        if previous_points:
            p1, st, err = cv2.calcOpticalFlowPyrLK(
                prev_gray, gray, LK_optical_flow_Lib.previous_points, None, **lk_params
            )
            good_new = p1[st == 1]
            ...
            for i, (new, old) in enumerate(zip(good_new, good_old)):
                a, b = new.ravel()
                c, d = old.ravel()
                LK_optical_flow_Lib.mask_image = cv2.line(
                    LK_optical_flow_Lib.mask_image, (a, b), (c, d), color[i].tolist(), 2
                )
...
class Dense_optical_flow_Lib(FunctionLibraryBase):
    ...
    def Dense_optical_flow(input=("ImagePin", None), img=(REF, ("ImagePin", None))):
        if (
            Dense_optical_flow_Lib.previous_image is None
