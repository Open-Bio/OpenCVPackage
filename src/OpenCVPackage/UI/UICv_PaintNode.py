...
from uflow.Core.Common import push, getConnectedPins
from ..UI.UIOpenCvBaseNode import UIOpenCvBaseNode
from ..CV_classes.imageUtils import *
from ..UI.pc_ImageCanvasWidget import toQImage
...
class PainterWidget(QtWidgets.QGraphicsPixmapItem):
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == QtCore.Qt.LeftButton and modifiers in [
...
class UICv_PaintMask(UIOpenCvBaseNode):
    ...
    def updateCanvasSize(self, pin):
        ...
        self._rawNode.IMAGE = expand_image_to_fit_rect(
            self._rawNode.IMAGE, (max(n_w, 1), max(n_h, 1))
        )
    ...
    def createInputWidgets(self, inputsCategory, inGroup=None, pins=True):
        inputsCategory.destroyed.connect(
            lambda x=None: self.removeItemFromViewer(inputsCategory)
        )
